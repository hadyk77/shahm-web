...
@section("content")
    ...
    <div class="modal fade" tabindex="-1" id="add_money_to_client" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded">
                <form action="" class="add_money_to_client_form" method="post">
                    @csrf
                    <div class="modal-header py-3">
                        <h3 class="modal-title" id="send_message_title">
                            {{__('Add Money to user wallet')}}
                        </h3>
                        <div class="btn btn-sm btn-icon btn-active-color-primary" data-bs-dismiss="modal">
                        <span class="svg-icon svg-icon-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
...
@section("scripts")
    ...
    <script>
        const add_money_to_client = document.getElementById('add_money_to_client')

        add_money_to_client.addEventListener('show.bs.modal', function (e) {
            $("#add_money_to_client").find('.add_money_to_client_form').attr('action', $(e.relatedTarget).data('url'));
        });

        $('#add_money_to_client .add_money_to_client_form').submit(function (e) {
            e.preventDefault();
            let form = $(this);
            let action = form.attr('action');
            $.ajax({
                ...
                success: function success(result) {
                    hideSpinner(form);
                    $('#add_money_to_client').modal('hide');
                    $("#datatables").DataTable().ajax.reload();
                    $('#add_money_to_client #amount').val("");
                    $('#add_money_to_client #notes').val("");
                    if (result.success) {
                        toastr.success(result.message);
                    } else {
                        toastr.error(result.message);
