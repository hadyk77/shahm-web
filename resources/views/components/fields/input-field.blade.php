@if($translatedInput)
    @foreach(["ar"] as $key)
        <div {{$attributes->merge(['style' => '', 'class' => 'col-md-' . ($col ?? '6') . ' form-group '])}}>
            <label class="d-flex align-items-center fs-5 mb-2" for="{{$name}}[{{$key}}]">{{$title}} {!!  $required ? '<sup>*</sup>' : '' !!}</label>
...
@else
    <div {{$attributes->merge(['style' => '', 'class' => 'col-md-' . ($col ?? '6') . ' form-group ', 'id' => $name . '_parent'])}}>
        <label class="d-flex align-items-center fs-5  mb-2" for="{{$name}}">{{$title}} {!!  $required ? '<sup>*</sup>' : '' !!}</label>
